...
data Config = Config
    { thresholdHigh :: Int
    -- ^ Enable when battery >= this
    , thresholdLow :: Int
    -- ^ Disable when battery < this
    , maxOutputWatts :: Double
    -- ^ Max watts per inverter
    , loadHeadroom :: Double
    -- ^ Headroom needed to enable switch
    , pvOnThreshold :: Double
    -- ^ Watts to consider PV as "on"
    , intervalEngaged :: Int
    -- ^ Microseconds when switches enabled
    , intervalIdle :: Int
    -- ^ Microseconds during solar window
    , intervalSleep :: Int
    -- ^ Microseconds at night
    , windowBufferMinutes :: Int
    -- ^ Start polling X minutes before sunrise
    , pvIncreaseTolerance :: Double
    -- ^ Tolerance for PV verification
    }
    deriving (Show, Eq)
...
data Action
    = -- | Enable switch i, record PV watts
      EnableSwitch Int Double
    | -- | Disable switch i
      DisableSwitch Int
    | -- | Verification passed
      VerifySuccess Int Double
    | -- | Verification failed
      VerifyFailed Int Double
    | UpdateSolarWindow SolarWindow Bool
    | -- | New day with new yesterday window
      DayRollover SolarWindow
    | NoAction
    deriving (Show, Eq)
...
applyAction action state = case action of
    EnableSwitch i pv ->
        state
            { switchesEnabled = setAt i True (switchesEnabled state)
    ...
    VerifyFailed i _ ->
        state
            { switchesEnabled = setAt i False (switchesEnabled state)
            , pending = Nothing
            }
